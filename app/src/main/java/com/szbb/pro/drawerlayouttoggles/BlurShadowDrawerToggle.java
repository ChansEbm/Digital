...
public class BlurShadowDrawerToggle implements DrawerToggle {
    ...
    public BlurShadowDrawerToggle(Activity a, DrawerLayout d, View behindDrawer, int gravity, int
            blurRadius) {
        this(a, d, behindDrawer, gravity);
        mBlurRadius = blurRadius;
    }

    public BlurShadowDrawerToggle(Activity a, DrawerLayout d, final View behindDrawer, int
            gravity) {
        mActivity = a;
        mDrawerLayout = d;
    ...
    public void onDrawerClosed(View arg0) {
        arg0.invalidateDrawable(mBlurDrawable); // reinvalidate the drawable, there are cases
        // when the pixel that previously were, are still visible.
        ...
        if (mMinusShadow == 0.0f) {
            if (mMinusShadow == 0.0f) {
                for (int i = 0; i < mDrawerLayout.getChildCount(); i++) {
                    mMinusShadow = mMinusShadow == 0.0f ? mDrawerLayout.getChildAt(i)
                            .getMeasuredWidth() : mMinusShadow;
                    mMinusShadow = Math.min(mMinusShadow, mDrawerLayout.getChildAt(i)
                            .getMeasuredWidth());
                }
    ...
    private void blurFirstTime() {
        ...
        mDrawable = Bitmap.createBitmap(mBehindDrawerView.getWidth(), mBehindDrawerView.getHeight
                (), Config.ARGB_8888);
...
//	        Log.i("blurOverlay ", "Entry; Radius: 	"+radius);
            ...
            Allocation input = Allocation.createFromBitmap(renderScriptSupport, bitmap,
                    Allocation.MipmapControl.MIPMAP_NONE, Allocation.USAGE_SCRIPT);
            Allocation output = Allocation.createTyped(renderScriptSupport, input.getType());
            ScriptIntrinsicBlur script = ScriptIntrinsicBlur.create(renderScriptSupport,
                    Element.U8_4(renderScriptSupport));
        ...
        protected Bitmap doInBackground(Bitmap... arg0) {
            ...
            //Log.i("AsyncBlur doInBackground()", " Time took to blur image, delta: "+ (System
