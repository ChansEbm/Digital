...
public class SimplePhotoView extends SimpleDraweeView implements View.OnTouchListener {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isCanDrag) {
                    if (getDrawable() != null) {
                        ...
                        //  if it is not wide enough for the screen, then the left and right
                        // movement is forbidden.
                        if (rectF.width() < getWidth()) {
                            isCheckLeftAndRight = false;
                        }
                        //  If it is not high enough for the screen, then the up and down
                        // movement is forbidden.
                        ...
                        checkMatrixBounds();
                        // Please pay attention here, after Matrix.postTranslate, do not use
                        // setImageMatrix(matrix). Use the method of invalidate().
                        // Becuase the drawee will not extend the ImageView, there is no way to
                        // make ImageView functional.
                        invalidate();
                    }
                }
                mLastX = x;
    ...
    /**
     * when  During the move, the bounded is checked. The width and height which is oversized of
    ...
    private class AutoRunableZoom implements Runnable {
        ...
        // input the target scale. According to the target scale and the current scale, to become
        // smaller or bigger is checked.
        ...
        public void run() {
            //  zoom in and out
            // please pay attention here. Do not use setImageMatrix(matrix) after Matrix
            // .postScale. Use the method of invalidate()
            ...
            invalidate();
            mCurrentScale = getScale() > SCALE_MAX ? SCALE_MAX : getScale() < 1.0f ? 1.0f :
                    getScale();
