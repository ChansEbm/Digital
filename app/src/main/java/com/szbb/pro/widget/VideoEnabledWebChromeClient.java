...
public class VideoEnabledWebChromeClient
        ...
        implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer
        .OnErrorListener {
    ...
    private boolean isVideoFullscreen; // Indicates if the video is being displayed using a
    // custom view (typically full-screen)
     ...
     * This constructor allows this class to be defined as an inline inner class in which the
     * user can override methods
     */
    ...
    /**
     * Set a callback that will be fired when the video starts or finishes displaying using a
     * custom view (typically full-screen)
     *
        ...
        if (view instanceof FrameLayout) {
            ...
            if (focusedChild instanceof android.widget.VideoView) {
                ...
                if (webView != null && webView
                    ...
                    // Run javascript code that detects the video end and notifies the Javascript
                    // interface
                            ...
                            "_ytrp_html5_video_last) {";
                    {
                        ...
                        {
                            js += "_VideoEnabledWebView.notifyVideoEnd();"; // Must match
                            // Javascript interface name and method of VideoEnableWebView
                        }
                        js += "}";
                        js += "_ytrp_html5_video.addEventListener('ended', " +
    ...
    public void onHideCustomView() {
        // This method should be manually called on video end in all cases because it's not
        ...
        if (isVideoFullscreen) {
            ...
            // Call back (only in API level <19, because in API level 19+ with chromium webview
            // it crashes)
            if (videoViewCallback != null && !videoViewCallback
                    .getClass()
                    .getName()
                    .contains(".chromium.")) {
    ...
    @Override
    public void onPrepared(MediaPlayer mp) // Video will start playing, only called in the case
    ...
    @Override
    public void onCompletion(MediaPlayer mp) // Video finished playing, only called in the case
    // of android.widget.VideoView (typically API level <11)
    {
        onHideCustomView();
    }
    ...
    @Override
    public boolean onError(MediaPlayer mp, int what, int extra) // Error while playing video,
    // only called in the case of android.widget.VideoView (typically API level <11)
    {
